		...
		if(data.needExplain()) {
			...
			for (String tblname : data.tables()) {
				...
				if (viewdef != null)
					...
					plans.add(plan);
					explain.add("-> TablePlan (#blks=" + plan.blocksAccessed() + ", #recs=" + plan.recordsOutput() + ")");
				}
			}
			...
			for (Plan nextplan : plans) {
				...
				explain.add("-> ProductPlan (#blks=" + p.blocksAccessed() + ", #recs=" + p.recordsOutput() + ")");
			}
			// Step 3: Add a selection plan for the predicate
			p = new SelectPlan(p, data.pred());
			explain.add("-> SelectPlan (#blks=" + p.blocksAccessed() + ", #recs=" + p.recordsOutput() + ")");
